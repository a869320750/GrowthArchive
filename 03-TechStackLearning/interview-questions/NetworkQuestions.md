...
select、poll、epoll 都是 Linux 下用于处理 I/O 多路复用的机制。select 能检测多个文件描述符的状态，但有一些限制，比如文件描述符数量有限制，效率也会随着数量增加而降低。poll 和 select 类似，但对文件描述符数量限制没那么严格。epoll 是更高效的方式，它采用事件驱动，能更好地处理大量并发连接，性能更优。简单来说，select 像个小管家，管理有限的事务还凑合；poll 能管更多事，但方法有点老；epoll 则是个高效能手，处理大量事务游刃有余。它们各有特点，适用不同场景呢。
